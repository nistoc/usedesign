...
A variant (§5.2e). The desk's own door — a librarian acting for a named member — and its own audit
line are proven by its own two tests; the four steps past the door are the original checkout's, run
by the same handler, and listed here under the original's ids so the card reads complete. Card
validation cannot see the original: it checks only that every inherited id is one of this card's
steps and that `shared` is in this card's own evidence. No `step_unproven` warning for the inherited
four — whether the original proves them is check 2's question.
