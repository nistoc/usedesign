...
variant_of:
  ...
  inherits: [s1-copy-available, s2-member-found, s3-member-standing, s4-loan-limit, s5-commit]
...
Inherits `s2-member-found` without listing it among its steps. A variant that leaves inherited
steps out reads as a door with nothing behind it — and a reader of this card alone would never
learn the member lookup happens. The card must list every step it runs.
