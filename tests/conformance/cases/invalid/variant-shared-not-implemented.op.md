...
maturity: tested
maturity_evidence:
  implemented: src/loans/CheckoutOnBehalf.ts
  tested: 2 tests
...
Names `src/loans/CheckoutHandler.ts` as the code it shares with the original, but its own
`implemented` evidence does not list that file. The claim that the same code carries the
inherited steps has to be made in the evidence, where check 3 confirms the file exists.
